...
const DonorForm = ({ navigation }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!name || !phone || !address || !foodItem || !quantity) {
      ...
      if (formRef.current) {
        formRef.current.shake(800); // Shake animation on error
      }
      return;
    }
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Animatable.View ref={formRef} animation="fadeInUp" duration={800} style={styles.formContainer}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Food Item(s) (e.g., Rice, Lentils, Vegetables)"
            placeholderTextColor={COLORS.textSecondary}
            value={foodItem}
          ...
          <TextInput
            style={styles.input}
            placeholder="Quantity (e.g., 5 kg, 3 packets)"
            placeholderTextColor={COLORS.textSecondary}
            value={quantity}
          ...
          >
            <Text style={styles.buttonText}>Submit Donation Details</Text>
          </Pressable>
        </Animatable.View>
      </ScrollView>
    </KeyboardAvoidingView>
